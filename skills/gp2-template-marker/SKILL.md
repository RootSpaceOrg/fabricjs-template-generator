...
3b. **Leia o contexto antes de marcar qualquer elemento** — este passo é obrigatório:
   ...
   - Extraia também o **registro visual das imagens** de `## Registro visual das imagens` (`Registro global:`) e, por slide, o override `Registro de imagem:` em `## Plano de slides` quando presente. Esse registro é o **componente de estilo obrigatório** de toda `data-te-description` de imagem (passo 4.3) — é o elo que faz a imagem gerada pela lambda casar com o design dark/claro/premium do template. Se o visual-plan não declarar o registro (template antigo), **infira do background** do slide (escuro → "dark"; claro → "claro/editorial") e registre uma nota em `marker-audit.md`.
...
4. **Caminhe slide por slide** (cada `<section class="slide">`) e para cada elemento decida na hora:
   ...
   - **Para cada elemento `data-template-element="true"`**, siga obrigatoriamente:
     ...
     3. **Imagem**: a description de imagem tem **três eixos** (ver `references/element-descriptions.md` §"Foto contextual"):
        - **Papel narrativo (obrigatório)** — função no slide (`imagem de capa/abertura`, `de prova`, `de fechamento/CTA`, `apoio contextual`), do papel do slide, **não** do que está dentro do `<img>`.
        - **Estilo (obrigatório — herdado do registro visual)** — copie o registro visual do passo 3b: o `Registro de imagem:` do slide se houver, senão o `Registro global:` do visual-plan (ex: `estilo dark premium editorial, alto contraste`). **Isto é o elo que faz a imagem gerada pela lambda casar com o design do template.** Não invente o estilo nem o omita — um template dark cuja imagem não diz "dark" gera imagem que destoa do design.
        - **Tema (aberto)** — reflete a mensagem-chave do slide em runtime; **ignore o tema do placeholder** (é só preview de layout). Anti-pattern proibido: placeholder de café+mesa virando `"foto de mesa/objeto; sem pessoa"`, ou registro do template virando assunto travado (`"joelho translúcido glow neon"`).

        Bounds técnicas (sem texto na imagem, sem watermark) sim; tema/enquadramento abertos.

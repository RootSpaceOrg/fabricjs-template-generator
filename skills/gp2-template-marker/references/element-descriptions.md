...
| **Estilo (registro visual)** | a estética do template (dark/claro/premium) | **obrigatório — herdado do registro visual** | `estilo dark premium editorial, alto contraste`, `claro e arejado, luz natural suave`, `flat minimalista, fundo neutro claro` |
...
> **De onde vem o estilo (não invente):** o componente de estilo é **herdado literalmente** do `## Registro visual das imagens` do `visual-plan.md` (saída do `gp2-art-director`) — o `Registro de imagem:` do slide se houver, senão o `Registro global:`. O art-director deriva esse registro da paleta/preset do template (fundo escuro → "dark"; preset `editorial-premium` → "claro/editorial premium"). O marker **copia**, não decide. Para templates antigos sem registro no plano, infira do background do slide e registre nota no audit.

> **Por que separar estilo de tema:** o template tem uma identidade visual (ex: um carrossel dark-tech). Essa identidade é **estilo** e é obrigatória na description (`estilo dark premium editorial`) — senão a imagem gerada pela lambda destoa do design. Mas o **assunto** (joelho, mitocôndria, café numa mesa) é o que o designer pôs no placeholder só pra validar layout — **nunca** vai pra description. Travar o assunto quebra o template em todos os outros usos e desconecta a imagem da mensagem-chave de cada slide.

...
- **`reference-spec.md`** (somente em reference-driven mode) — contrato extraído da imagem de referência. O art-director já o traduziu no visual-plan, mas você pode re-inspecionar para detalhes visuais.
- Opcionalmente: imagens de referência anexadas pelo usuário.
...
| **Reference-driven mode** | Idem, mais `reference-spec.md` como contrato adicional de fidelidade visual. |
...
- **Tipografia:**
  ...
  - Se reference-driven: use as famílias nomeadas no `reference-spec.md`. Se não disponível, escolha a mais próxima da mesma categoria e anote em `notes.md`.

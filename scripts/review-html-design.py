...
def placeholder_svg_complexity(svg: str) -> tuple[bool, str]:
    # Any inline SVG as image placeholder is now disallowed.
    # The pipeline uses image-placeholder.b64.txt (pre-built PNG) or a public URL (picsum.photos).
    # Inline SVGs generated by the designer are unpredictable in complexity and color.
    low = svg.lower()
    forbidden = []
    ...
    if forbidden:
        return True, f'SVG placeholder contains forbidden features: {", ".join(sorted(set(forbidden)))}. Use image-placeholder.b64.txt or a picsum.photos URL instead.'
    if shape_count > 2 or path_count > 1:
        return True, f'SVG placeholder is too complex ({shape_count} shapes, {path_count} paths). Use the pre-built image-placeholder.b64.txt or a picsum.photos URL instead.'
    return False, ''

# Templates publicados pela fábrica

Consolidado em 2026-08-12 ao limpar `artifacts/runs`.
O `template_id` vivia no `run.json` de cada run; a pasta é descartável, o
registro não.

| template_id | run | pack | env | slides |
|---|---|---|---|---|
| `PmWN_-9k6mW3T27s08mTX` | bold-arco-1 | bold-educacional | dev | 7 |
| `JGDIJeYbmEfcPVw0jocdl` | bold-arco-3 | bold-educacional | dev | 3 |
| `sf363uSZ_IEyR7xcx6SHO` | bold-arco-5 | bold-educacional | dev | 5 |
| `QNV8b46fzXfqyKV0yoTZV` | bold-ilib-1 | bold-educacional | dev | 1 |
| `SrFuch2cUDtMTGHIBcVtd` | bold-teste-1 | bold-educacional | dev | 1 |
| `o_r7z4f8f8dsuFFSLZgLj` | bold-teste-2 | bold-educacional | dev | 1 |
| `4xLNwys9mQAkfL6Hvx5qy` | cert-bold-v2-mito-3 | bold-educacional | dev | 3 |
| `DfoukJka-RVCN3BIaKHa8` | cert-bold-v2-sessoes-7 | bold-educacional | dev | 7 |
| `TyWivbiQNm3ByrPJ_IkCk` | cert-bold-v3-mito-5 | bold-educacional | dev | 5 |
| `L2feAqqwyKVxKsvQGadLI` | run-escolha-um-tema-para-tod | bold-educacional | dev | 3 |
| `Q-MeoRoSPdlw5hJjxyYLw` | cert-clin-v4-joelho-6 | clinical-photo-editorial | dev | 6 |
| `pxNwZYXbSZEWe_uynro1s` | cert-clin-v4-primeira-7 | clinical-photo-editorial | dev | 7 |
| `RMaBi35zy497fg4WcQ8X2` | cert-clin-v4b-joelho-5 | clinical-photo-editorial | dev | 5 |
| `oqdfMhjUU_IxhPfoFCYDt` | cert-fibro-1 | clinical-photo-editorial | dev | 6 |
| `SkJslOi8MgHlUtDmG-7SQ` | fissura2-clinical | clinical-photo-editorial | dev | 6 |
| `1IQlPc6UMLkv9EFXE_Hyz` | fissura4-clinical | clinical-photo-editorial | dev | 6 |
| `JPVE8Z1UGbEneH7TVnIwg` | gram-teste-1 | clinical-photo-editorial | dev | 6 |
| `Tp3B86MK1Ym7QB3ExkdJx` | var-teste-1 | clinical-photo-editorial | dev | 7 |
| `9ffkleDcTFjZCqWe-sDso` | var-teste-2 | clinical-photo-editorial | dev | 6 |
| `Q-mAo5BSoYCfrgl3-pwcu` | cert-ecc-v7-postura-7 | editorial-cards-continuos | dev | 7 |
| `LaANCovWDqgfAtU4Ct5sL` | cert-ecc-v7-recuperacao-4 | editorial-cards-continuos | dev | 4 |
| `L5sk0prJZLoFy9kK3o8-F` | cert-ecc-v7-recuperacao-5 | editorial-cards-continuos | dev | 5 |
| `huBP-J1bRGApyrofT-Sra` | cert-ecc-v8-postura-7 | editorial-cards-continuos | dev | 7 |
| `BCVYdJBxP71nZYzHImxJo` | cert-ecc-v8-recuperacao-5 | editorial-cards-continuos | dev | 5 |
| `wbOJlMz0bSsz1TPfOEBeS` | pais-2026 | emotive-fullbleed-lettering | dev | 1 |
| `-6sw4mOkeIFRHCz3LpLvM` | pais-2026-b | emotive-fullbleed-lettering | dev | 1 |
| `UcKvSkZ9WfIWTsWjxBMS-` | setembro-amarelo | emotive-fullbleed-lettering | dev | 1 |

#!/usr/bin/env node
/**
 * GP3 — conversor fechado HTML → Fabric JSON (contrato: gp3/engine/CATALOG.md).
 *
...
(async () => {
  ...
  if (!slidesDir || !outDir) {
    console.error("uso: node gp3/engine/convert.js <slides-dir> <outdir> [--slug <slug>]");
    process.exit(2);
  }
  ...
  const packDir = path.join(REPO, "gp3", "packs", packSlug);
  ...
  if (allRejects.length) {
    console.error(`REJEITADO — ${allRejects.length} violação(ões) do contrato (gp3/engine/CATALOG.md):`);
  ...
  slides.forEach((sl, i) => {
    ...
    const doc = { version: "5.5.2", background: rgb2hex(sl.bg) || "#FFFFFF", objects,
      _meta: { slideIndex: i, sourceClaudeDesign: `gp3-convert:${packSlug}:${sl.templateName || "template"}` } };
  ...
  try {
    execFileSync("node", [path.join(REPO, "scripts", "center-clippable-images.js"), outDir], { stdio: "inherit" });
  } catch (e) {
